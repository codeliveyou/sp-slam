...
void Tracking::newParameterLoader(Settings *settings) {
    ...
    if(mSensor==System::STEREO || mSensor==System::IMU_STEREO)
        mpORBextractorRight = new SPextractor(nFeatures,fScaleFactor,nLevels,(float)fIniThFAST/255.0f,(float)fMinThFAST/255.0f,mSPEnginePath);

    if(mSensor==System::MONOCULAR || mSensor==System::IMU_MONOCULAR)
        mpIniORBextractor = new SPextractor(5*nFeatures,fScaleFactor,nLevels,(float)fIniThFAST/255.0f,(float)fMinThFAST/255.0f,mSPEnginePath);
